...
class DGCNN_clsfeat(nn.Module):
    def __init__(self, emb_dims, k, dropout, output_channels=40, FPFH_type=False):
        super(DGCNN_clsfeat, self).__init__()
        # self.args = args
        self.k = k
        ...
        self.bn2 = nn.BatchNorm2d(64)
        self.bn3 = nn.BatchNorm2d(128)
        self.bn_FPFH = nn.BatchNorm2d(128)
        self.bn4 = nn.BatchNorm2d(256)
        ...
        self.conv_FPFH = nn.Sequential(nn.Conv2d(64*2, 128, kernel_size=1, bias=False),
                                   self.bn_FPFH,
                                   nn.LeakyReLU(negative_slope=0.2))
    ...
    def forward(self, x):
        ...
        if self.FPFH_type:
            ...
            print('DGCNN_ featur shape 0',input2.shape)
            x = get_graph_feature(input2, k=self.k)     # (batch_size, 64, num_points) -> (batch_size, 64*2, num_points, k)
            print('DGCNN_ featur shape 1',x.shape)
            x = self.conv_FPFH(x)                       # (batch_size, 64*2, num_points, k) -> (batch_size, 128, num_points, k)
            print('DGCNN_ featur shape 2',x.shape)
            
            x5 = x.max(dim=-1, keepdim=False)[0]    # (batch_size, 128, num_points, k) -> (batch_size, 128, num_points)
            x = torch.cat((x1, x2, x3, x4, x5), dim=1)  # (batch_size, 64+64+128+256, num_points)
